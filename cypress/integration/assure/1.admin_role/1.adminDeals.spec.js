describe('Admin login', () => {
    ...
    it('should open any deal from Grid View', () => {
        ...
        cy.get('#deal-view').contains('Overview')
            ...
            //     cy.contains('search').click()
                // cy.get('#mat-input-2').type('pending')
                // const dealTitle = cy.get('[title]').first()
                // var dealTitle = cy.get('[title]').first()
                // var dealTitle = cy.get('[title]').first().then(() => Cypress.LocalStorage.get('[title]'))
                // cy.get('[title]').first().click()
                // cy.get('[style="display: flex; align-items: flex-end;"] > .gb-title').should(function($span) {
                //     var text = $span.text()
                //     expect(text).equal(dealTitle)
                // })       
            // })
    ...
    it('should check search functionality in List view', () => {
        ...
        cy.contains('search').click().wait(2000)
        cy.get('#mat-input-2').clear().type('onboarding')
